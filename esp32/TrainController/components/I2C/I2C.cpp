...
void I2C::beginTransaction() {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "i2c_master_start: rc=%d %s", errRc, esp_err_to_name(errRc));
	}
	m_directionKnown = false;
} // beginTransaction
...
void I2C::read(uint8_t* bytes, size_t length, bool ack) {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "i2c_master_read: rc=%d %s", errRc, esp_err_to_name(errRc));
	}
} // read
...
void I2C::write(uint8_t *bytes, size_t length, bool ack) {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "i2c_master_write: rc=%d %s", errRc, esp_err_to_name(errRc));
	}
} // write
